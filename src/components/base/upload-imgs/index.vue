...
<script>
import {
 getFileType, checkIsAnimated, isEmptyObj, createId 
} from './utils'
  ...
  methods: {
    ...
    async uploadImg(item) {
      ...
      if (this.remoteFuc && typeof this.remoteFuc === 'function') {
        const result = await new Promise((resolve) => {
          ...
          // promise 模式
          if (
            remoteFucResult != null
            && typeof remoteFucResult.then === 'function'
          ) {
            remoteFucResult
              .then((remoteData) => {
                resolve(remoteData || false)
    ...
    previewImg(data, index) {
      ...
      if (this[this.globalImgPriview]) {
        ...
        // element 原生粗糙模式
        this.$confirm(
          `<img src="${data.display}" style="width: 100%;" />`,
          '预览',
          {
    ...
    async validateImg(imgInfo) {
      ...
      if (rule.allowAnimated && rule.allowAnimated > 0) {
        if (imgInfo.isAnimated === null) {
          this.$message.error(
            '要进行是否动图验证需要配置 "animated-check" 属性为 true',
          )
        } else {
      ...
      if (rule.width) {
        if (imgInfo.width !== rule.width) {
          throw new Error(
            `"${imgInfo.name}"图像宽不符合要求, 需为${rule.width}`,
          )
        }
      } else {
      ...
      if (rule.height) {
        if (imgInfo.height !== rule.height) {
          throw new Error(
            `"${imgInfo.name}"图像高不符合要求, 需为${rule.height}`,
          )
        }
      } else {
