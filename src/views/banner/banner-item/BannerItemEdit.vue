        ...
        <el-col :lg="16" :md="20" :sm="24" :xs="24">
          <el-form :model="form" status-icon ref="form" label-width="100px" @submit.native.prevent>
            ...
            <el-form-item label="图片" prop="image">
              <upload-imgs ref="uploadEle" :value="initData" :maxNum="maxNum" />
            </el-form-item>
...
export default {
  ...
  methods: {
    async getBannerItem() {
      const res = await bannerItem.getBannerItemById(this.bannerItemId)
      this.form = res
      this.initData.push({ display: res.image, id: 1 })
    },
    async submitForm(formName) {
      try {
        ...
        const res = await bannerItem.updateBannerItem(this.form.id, params)
        if (res.error_code === 0) {
